...
chooseFile() {
  ...
  input.onchange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      this.formData.pdfFile = {
        ...
        size: file.size,
        base64: base64,
        // H5 无法直接获取文件路径用于 uploadFile，所以用 base64 方式
        h5File: file
      }
      uni.showToast({
        title: '文件选择成功',
  ...
  this.chooseFileWithPromise().then(fileInfo => {
    this.formData.pdfFile = {
      name: fileInfo.name,
      size: fileInfo.size,
      path: fileInfo.path  // APP端直接使用文件路径上传
    }
    uni.showToast({
      title: '文件选择成功',
      icon: 'success',
...
uploadFileWithPath(filePath, apiUrl, jobName = '', extraData = {}) {
  return new Promise((resolve, reject) => {
    const token = uni.getStorageSync('token')
    ...
    console.log('额外参数:', extraData)
    
    // 构建 formData，只有当 jobName 有值时才添加
    const formData = { ...extraData }
    if (jobName) {
      formData.job_name = jobName
    }
    ...
    uni.uploadFile({
      ...
      success: (res) => {
        ...
        try {
          ...
          responseData = { rawResponse: res.data }
        }
    ...
    async submitResumeAnalysis(formData, method) {
      try {
        ...
        if (method === 'user+position') {
          ...
          // #ifdef APP-PLUS
          // App 端使用 uni.uploadFile 直接上传文件路径
          if (formData.pdfFile?.path) {
            res = await this.uploadFileWithPath(
              formData.pdfFile.path,
              'http://39.106.72.110/api/ai/ask_by_pdf_job_name',
			  jobName
            )
          ...
          if (formData.pdfFile?.path) {
            res = await this.uploadFileWithPath(
              formData.pdfFile.path,
              'http://39.106.72.110/api/ai/ask_by_pdf_job_text',
    ...
    async submitResumeEvaluation(formData, method) {
      if (method === 'user') {
        ...
        // App 端使用 uni.uploadFile 直接上传文件路径
        if (formData.pdfFile?.path) {
          const res = await this.uploadFileWithPath(
            formData.pdfFile.path,
            'http://39.106.72.110/api/ai/resume_evaluation_text'
          )
          return res?.data?.evaluation || res?.evaluation || res?.data || res
        }
        // #endif
    ...
    async submitSuccessRate(formData, method) {
      if (method === 'pdf+position') {
        ...
        if (formData.pdfFile?.path) {
          const res = await this.uploadFileWithPath(
            formData.pdfFile.path,
            'http://39.106.72.110/api/ai/success_rate_pdf_job_name',
            jobName
          )
          return res?.data?.analysis || res?.analysis || res?.data || res
        }
        ...
        if (formData.pdfFile?.path) {
          const res = await this.uploadFileWithPath(
            formData.pdfFile.path,
            'http://39.106.72.110/api/ai/success_rate_pdf_job_text',
            formData.positionText
          )
          return res?.data?.analysis || res?.analysis || res?.data || res
        }
    ...
    async submitStudentPlan(formData, method) {
      ...
      if (method === 'pdf+position') {
        ...
        if (formData.pdfFile?.path) {
          const res = await this.uploadFileWithPath(
            formData.pdfFile.path,
            'http://39.106.72.110/api/ai/university_plan_pdf_job_name',
            jobName,
