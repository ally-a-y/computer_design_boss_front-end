  ...
  methods: {
    ...
    async getJobDetail() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/job/detail/job_detail_index.vue:185", "转换后的delivers:", res);
        this.jobDetail = res;
        ...
        common_vendor.index.__f__("error", "at pages/job/detail/job_detail_index.vue:210", "获取职位详情失败:", error);
    ...
    async getUserProfile() {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/job/detail/job_detail_index.vue:219", "从API获取用户信息:", user);
        this.userProfile = user;
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/job/detail/job_detail_index.vue:222", "获取用户信息失败:", error);
        const userInfoStr = common_vendor.index.getStorageSync("userInfo");
        common_vendor.index.__f__("log", "at pages/job/detail/job_detail_index.vue:225", "从本地存储获取用户信息:", userInfoStr);
        if (userInfoStr) {
          try {
            const userInfo = JSON.parse(userInfoStr);
            common_vendor.index.__f__("log", "at pages/job/detail/job_detail_index.vue:229", "解析后的用户信息:", userInfo);
            this.userProfile = userInfo;
          } catch (parseError) {
            common_vendor.index.__f__("error", "at pages/job/detail/job_detail_index.vue:232", "解析用户信息失败:", parseError);
          }
        }
      }
      common_vendor.index.__f__("log", "at pages/job/detail/job_detail_index.vue:236", "最终用户信息:", this.userProfile);
    },
    /* ================= 点击投递 ================= */
    applyForJob() {
      var _a;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return common_vendor.e({
    a: common_vendor.o($options.goBack, "56"),
    b: common_vendor.p({
      type: "back",
      size: "28"
    }),
    ...
    E: common_vendor.o((...args) => $options.favoriteJob && $options.favoriteJob(...args), "a1"),
    F: common_vendor.t($data.isApplied ? "已投递" : "投递"),
    G: common_vendor.n({
      "applied": $data.isApplied
    }),
    H: $data.isApplied,
    I: common_vendor.o((...args) => $options.applyForJob && $options.applyForJob(...args), "b0")
  });
}
