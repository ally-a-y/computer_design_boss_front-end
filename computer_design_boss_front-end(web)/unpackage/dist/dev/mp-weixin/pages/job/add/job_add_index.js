...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    a: common_vendor.o((...args) => $options.onCancel && $options.onCancel(...args), "85"),
    b: common_vendor.o((...args) => $options.onSubmit && $options.onSubmit(...args), "c0"),
    c: $data.jobData.title,
    d: common_vendor.o(($event) => $data.jobData.title = $event.detail.value, "82"),
    e: $data.jobData.company_id,
    f: common_vendor.o(($event) => $data.jobData.company_id = $event.detail.value, "18"),
    g: $data.jobData.city_id,
    h: common_vendor.o(($event) => $data.jobData.city_id = $event.detail.value, "74"),
    i: common_vendor.t($options.selectedCategoryName || "请选择职位分类"),
    j: $data.categoryList,
    k: $options.getCategoryIndex(),
    l: common_vendor.o((...args) => $options.onCategoryChange && $options.onCategoryChange(...args), "cd"),
    m: common_vendor.t($options.selectedEmpTypeName || "请选择就业类型"),
    n: $data.empTypeList,
    o: $options.getEmpTypeIndex(),
    p: common_vendor.o((...args) => $options.onEmpTypeChange && $options.onEmpTypeChange(...args), "87"),
    q: $data.jobData.salary_min,
    r: common_vendor.o(($event) => $data.jobData.salary_min = $event.detail.value, "23"),
    s: $data.jobData.salary_max,
    t: common_vendor.o(($event) => $data.jobData.salary_max = $event.detail.value, "fc"),
    v: $data.jobData.edu_req,
    w: common_vendor.o(($event) => $data.jobData.edu_req = $event.detail.value, "bd"),
    x: $data.jobData.exp_req,
    y: common_vendor.o(($event) => $data.jobData.exp_req = $event.detail.value, "94"),
    z: $data.jobData.description,
    A: common_vendor.o(($event) => $data.jobData.description = $event.detail.value, "2b")
  };
}
