<template>
  <view class="ai-chat-container" :class="currentTheme + '-theme'" :style="{ background: isDarkMode ? '#1a1a1a' : 'linear-gradient(135deg, #e6f0ff 0%, #ffffff 100%)' }">
    ...
    <!-- 对话区域 -->
    <scroll-view class="chat-area" scroll-y :scroll-top="scrollTop" @scrolltoupper="loadMoreHistory" :style="{ background: isDarkMode ? '#1a1a1a' : 'linear-gradient(135deg, #e6f0ff 0%, #ffffff 100%)' }">
  ...
  methods: {
    ...
    chooseFile() {
      ...
      input.onchange = (e) => {
        ...
        reader.onload = (event) => {
          // 获取 base64 数据（去掉 data:application/pdf;base64, 前缀）
      ...
      // #ifdef APP-PLUS
      // App 端使用 plus.io.chooseFile
      // 注意：需要配置权限，或者使用 uni.chooseFile（需要更高版本）
      // #ifdef APP-PLUS
      if (typeof plus !== 'undefined' && plus.io) {
        plus.io.chooseFile({
          ...
          success: (res) => {
            ...
            if (!filePath) return
            const fileName = String(filePath).match(/[^/\\]+\.pdf$/i)?.[0] || `file_${Date.now()}.pdf`
...
.chat-area {
  ...
  margin-bottom: 250rpx; // 调整底部外边距，适应功能栏变化
  height: calc(100vh - var(--status-bar-height) - 160rpx - 250rpx); // 计算准确的高度
  overflow: hidden; // 确保内容不会溢出
  
  .message-list {
    padding-bottom: 10rpx; /* 增加底部padding，确保消息胶囊底部有10px空间 */
    ...
    .message-item {
        ...
        .message-bubble {
          max-width: 72%;
          padding: 24rpx 28rpx; /* 增加上下内边距，增高消息胶囊高度 */
          border-radius: 24rpx;
...
.input-fixed {
  ...
  z-index: 99;
  background-color: #fff;
  padding: 16rpx 24rpx 36rpx 24rpx; /* 减小顶部padding，增加底部padding */
...
.panel-overlay {
  ...
  .function-panel {
    ...
    backdrop-filter: blur(12px);
    -webkit-backdrop-filter: blur(12px);
    margin-bottom: 0; // 移除底部外边距，确保面板紧贴屏幕底部
